...
class ImageProcessor implements OptimizerInterface
{
	...
	private function process_image_element($image)
	{
		...
		// Graceful degradation: if optimization is still in progress,
		// leave the original <img> tag as-is instead of wrapping in <picture>.
		$status_data = $this->image_model->get_status($attachment_id);
		if ($status_data && !in_array($status_data['status'], array('completed', 'none'), true)) {
			return;
		}
